...
parser.add_argument("--sleep", "-s", dest='sleep', help="Sleep this long between lines (in seconds)", default=0.0, type=float)
parser.add_argument("--req", "-r", dest='req', help="num of req per second", default=0, type=int)
...
while (flag):
    if not args.req:
        if args.sleep:
            ...
            increment = datetime.timedelta(seconds=random.randint(0, 60))
        otime += increment
    ...
    if reqs >= req_per_sec:
        reqs=0
        otime += datetime.timedelta(seconds=1)
    ...
    uri = "/"+faker.uri_path()+"/"+faker.uri_page()+faker.uri_extension()
    if uri.find("apps")>0:
